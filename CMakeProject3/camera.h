...
public:
    ...
    void render(const Scene& scene) {
        ...
        if (num_threads == 0) num_threads = 4; // fallback if unknown

        std::clog << "Rendering with " << num_threads << " threads...\n";

        // Divide image rows into thread chunks
                ...
                for (int j = start_row; j < end_row; j++) {
                    for (int i = 0; i < image_width; i++) {
                        ...
                        for (int sample = 0; sample < samples_per_pixel; sample++) {
                            ray r = get_ray(i, j);
                            pixel_color += path_trace(scene, r, max_depth);
                        }
                        framebuffer[j * image_width + i] = pixel_samples_scale * pixel_color;
                    }

                    // Progress output (atomic-safe)
                    lines_done++;
                    if (lines_done % 10 == 0) {
                        std::clog << "\rScanlines remaining: " << (image_height - lines_done) << ' ' << std::flush;
                    }
                }
                }));
        }
        ...
        // Write image (single-threaded, ordered)
